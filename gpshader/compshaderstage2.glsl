...
int triiter = 0;
bool test3 = false;
ivec3 posi = ivec3(gl_WorkGroupID.xyz);
vec3 pos = posi*scale;
bool test2 = false;
for(int trinum = 0; trinum < tracker[9]; trinum++){
	test2 = false;
	vec3 triangle[3] =vec3[] (Faces[trinum*3].xyz,Faces[trinum*3+1].xyz,Faces[trinum*3+2].xyz);
	vec3 a = rot[trinum].xyz;
	float theta = rot[trinum].w;
	mat3 id = mat3(1.0);
	mat3 om = mat3(
		0,-a.z,a.y,
		a.z,0,-a.x,
	...
	vec3 xo = vec3(0.5,0,0)*scale;
	vec3 yo = vec3(0,0.5,0)*scale;
	vec3 zo = vec3(0,0,0.5)*scale;
	...
	/*
		if(posi.z  < posi.y && posi.x ==0){
			int tracloc = atomicAdd(tracker[7],2);
			floatd[tracloc].xyz = rotation*posi;// gl_WorkGroupID.xyz;
			floatd[tracloc].w = -10;
			floatd[tracloc+1].xyz = posi;
			if(test1 == true){
				floatd[tracloc+1].w = 1;
			}
		}
	*/
	if(test1 == true){
	...
	/*
		int tracloc = atomicAdd(tracker[7],8);
					for(int yyy = 0; yyy< 7;yyy++){

					floatd[tracloc+yyy].xyz = corner[yyy];
		
				}
	
				floatd[tracloc+7] = vec4(1000,1000,10000,10000);
	*/
	// calculate the boundary of cube intersect with triangle;
		...
		*/
		...
		test2 = check_engulf(mostdistpos, count, boundaries,centreline, triangle);
	/////////////////////////////////////////////////////
...
if(test2 == true){
		if(trinum == 1){
			...
			//floatd[inc].xyz = posi.xyz;
			//floatd[inc].w = inc;
			//if(posi.x == 0 && posi.y == 8 && posi.z == 8){
				//inc = atomicAdd(intd[0].x,2);
				//floatd[inc].x = triiter;
				//floatd[inc].w = 1011;
			//}
		}
	test3 = true;
	set_trilist1(posi.x,posi.y,posi.z,trinum);
	triiter++;
}
}
if(test3 == true){
	set_cubelist1(posi.x, posi.y,posi.z);
	/*
	if(posi.x >){
	int local = atomicAdd(tracker[8],1);
	intd[local].x = posi.x;
	intd[local].y = cubepos1[tracker[0]-1].x;
	intd[local].w = tracker[0];
	}*/
}
}