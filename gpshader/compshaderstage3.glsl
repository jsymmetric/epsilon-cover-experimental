...
void set_cubelist2(int x,int y, int z){
	//if(x != 0 || y != 0 || z != 0){
		...
		//int tracloc = atomicAdd(tracker[7],1);
		memoryBarrierBuffer();
		//floatd[tracloc].w = pos;
		//floatd[tracloc].z = 10;

	    cubepos2[pos].xyz = ivec3(x,y,z);
	//}
};
...
ivec3 posi = ivec3(gl_LocalInvocationID.xyz);
ivec3 GlobalPos = get_cubelist1(tracker[3]);
vec3 pos = (GlobalPos+posi*0.1)*scale;
...
do{
	...
	if(test1 == 1){
		...
		vec4 intersection[10];
			...
			*/
			...
			if(check_engulf(mostdistpos, count, boundaries,centreline, triangle) == true){
				test2 = 1;
			}
	...
	trinum++;
	tricheck = get_trilist1(GlobalPos.x,GlobalPos.y,GlobalPos.z,trinum);
...
//}}}
if(test3 == 1){
if(GlobalPos.z == 8 && GlobalPos.x > 0 && posi.z != 3){
	//int tracloc = atomicAdd(tracker[7],2);
	//floatd[tracloc].xyz = vec3(posi);
}	
	set_cubelist2(posi.x,posi.y,posi.z);
